...
import iti.mad.marketly.helpers.AdsManager

import iti.mad.marketly.presentation.view.adapter.BrandsAdapter
import iti.mad.marketly.presentation.view.states.AdsStats
import iti.mad.marketly.presentation.view.states.BrandApiStatus
import iti.mad.marketly.presentation.view.states.PricingRuleState
import iti.mad.marketly.presentation.view.viewmodel.AdsViewModel
        ...
        lifecycleScope.launch(Dispatchers.Main){
            repeatOnLifecycle(Lifecycle.State.STARTED){
                adsViewModel._discount.collect{
                    when(it){
                        is AdsStats.Loading ->{
                            Toast.makeText(requireContext(),"LOOOOOAAADO",Toast.LENGTH_LONG).show()
                        }
                        is AdsStats.Success ->{
                            Toast.makeText(requireContext(),"${it.discountResponce.discount_codes.size}",Toast.LENGTH_LONG).show()
                            AdsManager.addDiscountList(it.discountResponce.discount_codes)
                            val imgList=ArrayList<SlideModel>()
                            imgList.add(SlideModel("https://cdn.vectorstock.com/i/1000x1000/01/58/summer-sales-seasonal-special-offer-advertisement-vector-25930158.webp", "Summer Sales",
                                ScaleTypes.FIT))
                            imgList.add(SlideModel("https://cdn.vectorstock.com/i/1000x1000/25/02/great-winter-sales-neon-banner-vector-44882502.webp", "Summer Sales",ScaleTypes.FIT))
                            binding.imageSlider.setImageList(imgList)
                            binding.imageSlider.setItemClickListener(object : ItemClickListener {
                                override fun onItemSelected(position: Int) {
                                    AdsManager.setClipBoard(AdsManager.adsList.get(0).code)
                                    Toast.makeText(requireContext(),AdsManager.adsList.get(0).code,Toast.LENGTH_LONG).show()
                                    if(AdsManager.useCode(AdsManager.adsList.get(0).code)){
                                        Toast.makeText(requireContext(),"You can use The code",Toast.LENGTH_LONG).show()
                                    }else{
                                        Toast.makeText(requireContext(),"You can't use The Code",Toast.LENGTH_LONG).show()
                                    }
                                }
                            })
                        }
