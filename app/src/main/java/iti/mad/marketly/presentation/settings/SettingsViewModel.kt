...
class SettingsViewModel(private val repo:SettingsRepoInterface):ViewModel() {
    private var currency : MutableStateFlow<ResponseState<CurrencyResponse>> = MutableStateFlow(ResponseState.OnLoading())
    val _currency: StateFlow<ResponseState<CurrencyResponse>> = currency
    private var addressResponse : MutableStateFlow<ResponseState<List<iti.mad.marketly.data.model.settings.Address>>> = MutableStateFlow(ResponseState.OnLoading())
    val _addressResponse: StateFlow<ResponseState<List<iti.mad.marketly.data.model.settings.Address>>> = addressResponse
    fun getExchangeRate(){
